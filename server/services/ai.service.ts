...
/**
 * Service for handling AI operations using Google's Generative AI
 */
export class AIService extends BaseService {
  ...
  constructor() {
    ...
    this.systemInstruction = this.getSystemInstructions();
  }
  ...
  async initialize(): Promise<void> {
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.warn("GEMINI_API_KEY not found. AI service will not function properly.");
    }
  }
  ...
  async generateCareerRecommendations(
    ...
    try {
      ...
      const prompt = this.createPrompt(assessmentData);
  ...
  /**
   * Creates a prompt for the AI model based on assessment data
   * @param assessmentData User's assessment data
   * @returns Formatted prompt string
   */
  private createPrompt(assessmentData: AssessmentData): string {
    return `
  ...
  /**
   * Returns the system instructions for the AI model
   */
